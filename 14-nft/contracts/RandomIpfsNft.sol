...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    function fulfillRandomWords(
        ...
        address owner = s_requestId2Sender[requestId];
        uint256 newTokenId = s_tokenCounter;

        uint256 rng = randomWords[0];
        Breed breed = getBreedFromRandomNumber(rng);
    ...
    function getBreedFromRandomNumber(uint256 rng) public pure returns (Breed) {
        uint256 cumulativeSum = 0;
        uint256[3] memory chanceArray = getChanceArray();
        rng = rng % MAX_CHANCE_VALUE;
